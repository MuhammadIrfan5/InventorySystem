                                ...
                                @foreach ($inventories as $key => $inventory)
                                    ...
                                    @if ($key % 7 == 0)
                                        <tr>
                                            ...
                                            <td><img src="data:image/png;base64,{{DNS2D::getBarcodePNG(\Illuminate\Support\Facades\URL::signedRoute('inventory_detail',['inv_id' => $inventory->inventory_id]), 'QRCODE')}}" alt="barcode" style="width: 80px; height: 80px;" /> <br/>
                                                @if(strlen($inventory->inventory->product_sn) > 7)
                                                    <span>{{ substr($inventory->inventory->product_sn,strlen($inventory->inventory->product_sn)-7,strlen($inventory->inventory->product_sn)) }}</span>
                                                @else
                                                    <span>{{ $inventory->inventory->product_sn }}</span>
                                                @endif
                                            </td>
                                            @if (($key + 1) % 7 == 0)
                                        </tr>
